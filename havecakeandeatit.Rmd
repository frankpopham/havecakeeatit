...
t3marg <- bind_cols(tmarg, t2marg) %>%
  ...
  rename("95% CI - Low"="2.5 %",
         "95% CI - High"="97.5 %") %>%
  select(X, everything()) %>%
  mutate(across(-c(X, Effect), ~if_else(Effect=="OR", exp(.x), .x)))
...
t3con <- bind_cols(tcon, t2con) %>%
  ...
  rename("95% CI - Low"="2.5 %",
         "95% CI - High"="97.5 %") %>%
  select(X, everything()) %>%
  mutate(across(3:5, ~if_else(Effect=="OR", exp(.x), .x)))
...
fscrr <- svycontrast(coss, quote((exp(co1)/(1+exp(co1)))/(exp(co0)/(1+exp(co0)))))
...
fsmrr <- svycontrast(fsmor, quote(((exp(`(Intercept)`+`factor(X)1`))
                                  / (1+(exp(`(Intercept)`+`factor(X)1`)))) / 
